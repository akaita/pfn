...
*
* Carga la pantalla menú de administración de usuarios
*******************************************************************************/
...
if ($PFN_vars->post('accion') == 'gardar') {
	...
	if (is_array($antes)) {
		foreach ($antes as $k => $v) {
			...
			if ($v != $estado[$k]) {
				if (($estado[$k] == 0) && ($sPFN['usuario']['id'] == $k)) {
					$erros[] = 10;
				} else {
					$PFN_usuarios->accion('estado', 'usuarios', $estado[$k], $k);
				}
			}
		}
...
$PFN_usuarios->init('usuarios_total', $buscar);
...
$PFN_usuarios->init('usuarios_pax', $paxina, $paxinar, $buscar);
