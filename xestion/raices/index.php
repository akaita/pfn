...
*
* Carga la pantalla menú de administración de raices
*
...
if ($PFN_vars->post('accion') == 'gardar') {
	...
	if (is_array($antes)) {
		foreach ($antes as $k => $v) {
			...
			if ($v != $estado[$k]) {
				if (($estado[$k] == 0) && ($sPFN['raiz']['id'] == $k)) {
					$erros[] = 3;
				} else {
					$PFN_usuarios->accion('estado', 'raices', $estado[$k], $k);
				}
			}
		}
...
$PFN_usuarios->init('raices_total', $buscar);
...
$PFN_usuarios->init('raices_pax', $paxina, $paxinar, $buscar);
